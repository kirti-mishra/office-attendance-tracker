...
def best_8_week_attendance(attendance, reference_date):
    ...
    best_8 = sorted(summary.values(), reverse=True)[:8]
    return sum(best_8), summary
...
with col_calendar:
    st.header("🗓️ Mark Attendance and OOO (Past + Future)")

    # Initialize toggle state if not present
    if "show_monthly_totals" not in st.session_state:
        st.session_state.show_monthly_totals = False

    # Toggle button
    if st.button("📊 Toggle Monthly Totals"):
        st.session_state.show_monthly_totals = not st.session_state.show_monthly_totals

    # Show the monthly totals if toggled on
    if st.session_state.show_monthly_totals:
        month_totals = defaultdict(lambda: {"office": 0, "ooo": 0})
        combined_weeks = generate_weeks(selected_future_week - timedelta(weeks=1), 12)[::-1] + \
                        [get_week_start(selected_future_week + timedelta(weeks=w)) for w in range(0, 6)]

        for week_start in combined_weeks:
            for i in range(5):
                day = week_start + timedelta(days=i)
                month_key = datetime(day.year, day.month, 1).date()  # Use first of the month for sorting
                if day in st.session_state.attendance:
                    month_totals[month_key]["office"] += 1
                if day in st.session_state.ooo:
                    month_totals[month_key]["ooo"] += 1

        for month_key in sorted(month_totals.keys()):
            label = month_key.strftime("%B %Y")
            counts = month_totals[month_key]
            st.markdown(f"**{label}**: 🏢 {counts['office']} | 🏖️ {counts['ooo']}")
...
with col_summary:
    ...
    past_days, past_summary = best_8_week_attendance(st.session_state.attendance, selected_future_week)
    ...
    if total_projected >= POLICY_DAYS_REQUIRED:
        ...
        df_long = df.melt(id_vars="Week", value_vars=["Office Days", "OOO Days", "Suggested Days"],
        ...
        fig = px.bar(
            ...
            color="Type",
            barmode="stack",
            title="Office vs OOO Days (12 Weeks Prior + 6 Weeks Future)",
